...
const handleEditService = async () => {
  loading.value = true;
  try {
    const edition = await editService(service.value.id, {
      ...service.value,
      image: editServiceImage.value.file || service.value.image,
    });
    if (edition.status) {
      loading.value = false;
      setNotification({
...
<template>
  ...
  >
    ...
    <FormLoader v-if="serviceLoading" />
    <template v-else>
      <img
        class="rounded-md object-contain w-[60px] mb-3"
        :src="service.image"
        :alt="
          service.image_description ? service.image_description : service.title
        "
        v-if="service.image"
      />
      <form
      ...
      >
        ...
        <div class="my-3">
          ...
          <div class="mt-4 mb-2" v-if="editServiceImage.preview !== null">
            <img
              class="rounded-md object-contain w-[400px] p-4"
